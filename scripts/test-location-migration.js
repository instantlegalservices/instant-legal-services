...
/*
 * --------------------------------------------------------------------------
 * 59. Migration summary contract integrity
 * --------------------------------------------------------------------------
 */

test(
  "migration summary exposes the complete required contract",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "migration summary reports correct counts for one route change",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "migration summary counts match actual output collections",
  () => {
    const result =
      migrateLocations(
        [
    ...
    assert.strictEqual(
      result.summary.historicalRoutes,
      result.migratedManifest.reduce(
        (total, entry) =>
          total +
          entry.previousRoutes.length,
        0
      )
    );
...
test(
  "no-op migration reports zero migrations and zero redirects",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "empty migration reports zero changes while preserving current routes",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "historical route count reflects complete migration history",
  () => {
    const first =
      migrateLocations(
        [
    ...
    assert.strictEqual(
      second.summary.migrated,
      1
    );

    assert.strictEqual(
      second.summary.redirectsRequired,
      1
    );

    assert.strictEqual(
      second.summary.currentRoutes,
      1
    );

    assert.strictEqual(
      second.summary.historicalRoutes,
      2
    );

    assert.strictEqual(
      second.summary.rollbackReady,
      true
    );
...
/*
 * --------------------------------------------------------------------------
 * 60. Summary vs manifest integrity
 * --------------------------------------------------------------------------
 */

test(
  "summary historicalRoutes exactly matches manifest history",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "summary historicalRoutes increases by exactly one after a route migration",
  () => {
    ...
    assert.strictEqual(
      before.summary.historicalRoutes,
      1
    );

    assert.strictEqual(
      after.summary.historicalRoutes,
      2
    );

    assert.strictEqual(
      after.summary.historicalRoutes,
      before.summary.historicalRoutes + 1
    );
  }
);

test(
  "summary currentRoutes remains equal to manifest size after sequential migrations",
  () => {
    const first =
      migrateLocations(
        [
    ...
    assert.strictEqual(
      first.summary.currentRoutes,
      first.migratedManifest.length
    );

    assert.strictEqual(
      second.summary.currentRoutes,
      second.migratedManifest.length
    );
...
test(
  "summary migrated equals redirect count for sequential migration",
  () => {
    const first =
      migrateLocations(
        [
...
test(
  "summary currentRoutes counts only current routes, never historical routes",
  () => {
    const result =
      migrateLocations(
        [
...
test(
  "successful migration summary always reports rollbackReady true",
  () => {
    const result =
      migrateLocations(
        [
