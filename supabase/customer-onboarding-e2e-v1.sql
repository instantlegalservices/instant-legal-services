-- CUSTOMER ONBOARDING E2E V1
-- TEST-only controlled onboarding boundary.
-- No schema changes. No service_requests writes.
-- Customer identity is always derived from auth.uid().
...
begin
  ...
  if v_passport_id is not null then
    customer_id := v_user_id; passport_id := v_passport_id; status := v_status;
    created := false; environment := 'TEST'; return next; return;
  end if;
  ...
  customer_id := v_user_id; passport_id := v_passport_id; status := v_status;
  created := coalesce(v_created, false); environment := 'TEST'; return next;
end;
$function$;
