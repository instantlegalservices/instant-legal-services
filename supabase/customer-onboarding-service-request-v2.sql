-- CUSTOMER ONBOARDING E2E V2
-- TEST-only controlled Customer onboarding completion.
-- Extends the existing V1 boundary; no schema changes and no RLS weakening.
-- The browser supplies no user_id: identity is always auth.uid().
-- The function creates the Customer-owned service_request inside the same transaction.
...
begin
  ...
  then
    raise exception 'TEST_CUSTOMER_ONBOARDING_BOUNDARY_NOT_AVAILABLE';
  end if;

  if nullif(btrim(p_complaint_token), '') is null then
    raise exception 'COMPLAINT_TOKEN_REQUIRED';
  end if;

  if nullif(btrim(p_complaint_summary), '') is null then
    raise exception 'COMPLAINT_SUMMARY_REQUIRED';
  end if;

  if nullif(btrim(p_service_code), '') is null then
    raise exception 'SERVICE_CODE_REQUIRED';
  end if;

  if p_required_documents is null
     or jsonb_typeof(p_required_documents) <> 'array'
  then
    raise exception 'REQUIRED_DOCUMENTS_MUST_BE_ARRAY';
  end if;

  /*
    Resolve only an active service. For the current isolated TEST project,
    the application-facing LEGAL_ASSISTANCE code is explicitly mapped to
    the existing TEST-only service catalog row. No Production catalog is
    touched and no service price/payment state is created.
  */
  select s.id
    into v_service_id
  ...
  if v_service_id is null and upper(btrim(p_service_code)) = 'LEGAL_ASSISTANCE' then
    select s.id
      into v_service_id
    from public.services s
    where s.slug = 'synthetic-legal-service'
      and lower(btrim(s.status)) = 'active'
    limit 1;
  end if;
  ...
  /*
    Complaint token is the idempotency key for the onboarding transaction.
    If the passport already belongs to this authenticated user, reuse it
    and ensure the corresponding owned pending service request exists.
  */
  select cp.id, cp.status
    into v_passport_id, v_status
  ...
  /*
    The passport and service request are created in the same transaction.
    If the request insert fails, the passport creation also rolls back.
  */
  insert into public.service_requests(user_id, service_id, status)
  values (v_user_id, v_service_id, 'pending');
